...
pub const Document = struct {
    ...
    pub fn save(self: Document, project: *Project) !void {
        const filePath = project.assetIndex.getIndex(self.getId()) orelse std.debug.panic("Could not save document {f}: Could not find index", .{self.getId()});
        const state = self.state catch |err| return err;
...
pub const DocumentContent = union(DocumentTag) {
    ...
    pub fn serialize(self: DocumentContent, writer: *std.Io.Writer) !void {
        switch (self) {
